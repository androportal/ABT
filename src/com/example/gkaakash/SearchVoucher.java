...
public class SearchVoucher extends Activity {
		...
		vouchertable.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				if (oneTouch == 1) {
					...
					final SpannableString rsSymbol = new SpannableString(SearchVoucher.this
					        .getText(R.string.Rs));
					/** Create a TableRow dynamically **/
					if ("Journal".equals(vouchertypeflag) || "Credit note".equals(vouchertypeflag)
					        || "Debit note".equals(vouchertypeflag)) {
						ColumnNameList = new String[] { "V. No.", "Reference No", "Date",
						        "Voucher Type", "Account Name", "Particular", rsSymbol + "Amount",
						        "Narration" };
	...
	private void setOnSearchButtonClick() {
		...
		btnSearchVoucher.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				View layout = inflater.inflate(R.layout.search_voucher_by,
				        (ViewGroup) findViewById(R.id.timeInterval));
				...
				final EditText voucher_number = (EditText) layout
				        .findViewById(R.id.etVoucher_number);
				...
				final LinearLayout timeInterval = (LinearLayout) layout
				...
				searchBy.setOnItemSelectedListener(new OnItemSelectedListener() {
					...
					public void onItemSelected(AdapterView<?> parent, View v, int position, long id) {
						...
						// ArrayAdapter<String>(SearchVoucher.this,
						...
						if (position == 4) {
							...
							// Object[] subgroupnames =
							// (Object[])group.getSubGroupsByGroupName(params,client_id);
							...
							ArrayAdapter<String> adapter = new ArrayAdapter<String>(
							        SearchVoucher.this,
							        android.R.layout.simple_spinner_dropdown_item, accountnamelist);
							group_name.setAdapter(adapter);
						...
						if (position == 5) {
							...
							List<String> voucherTypes = new ArrayList<String>(Arrays
							        .asList(new String[] { "Contra", "Journal", "Payment",
							                "Receipt", "Credit note", "Debit note", "Sales",
							                "Sales return", "Purchase", "Purchase return" }));

							ArrayAdapter<String> adapter = new ArrayAdapter<String>(
							        SearchVoucher.this,
							        android.R.layout.simple_spinner_dropdown_item, voucherTypes);
				...
				btnView.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View arg0) {
						...
						if (pos == 0) {
							...
							Boolean validateDateFlag = m.validateDate(financialFromDate,
							        financialToDate, from_btn_ID.getText().toString(), to_btn_ID
							                .getText().toString(), "validatebothFromToDate",
							        tvWarning);
							if (validateDateFlag) {
								givenfromDateString = m.givenfromDateString;
								givenToDateString = m.givenToDateString;
								searchVoucherBy = 2; // by date
								Object[] params = new Object[] { 2, "", givenfromDateString,
								        givenToDateString };
								getallvouchers(params);
							}
						} else if (pos == 2) {
							...
							Object[] params = new Object[] { 5, financialFromDate, financialToDate,
							        Account_Name };
							getallvouchers(params);
							validate = true;
						} else if (pos == 5) {
							...
							Object[] params = new Object[] { 6, financialFromDate, financialToDate,
							        vouchertypeflag };
							getallvouchers(params);
							validate = true;
						}
		...
		for (int i = 0; i < searchedVoucherGrid.size(); i++) {
			...
			for (int j = 0; j < columnValue.size(); j++) {
				...
				if ((i + 1) % 2 == 0)
					...
					label.setBackgroundColor(Color.parseColor("#085e6b")); // blue
																		   // theme
				/*
				 * set center aligned gravity for amount and for others set
				 * center gravity
				 */
				if (j == 6) {
			...
			// Add the TableRow to the TableLayout
			vouchertable.addView(tr, new TableLayout.LayoutParams(LayoutParams.FILL_PARENT,
	...
	public void addHeader() {
		...
		if ("Journal".equals(vouchertypeflag) || "Credit note".equals(vouchertypeflag)
		        || "Debit note".equals(vouchertypeflag)) {
			ColumnNameList = new String[] { "V. No.", "Reference No", "Date", "Voucher Type",
			        "Account Name", "Particular", rsSymbol + "Amount", "Narration" };

		} else {
			ColumnNameList = new String[] { "V. No.", "Reference No", "Date", "Voucher Type",
			        "Account Name", "Particular", rsSymbol + "Amount", "Narration", "Cheque No." };
	...
	public void addRow(String string, final int i) {
		...
		tr.setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				// Toast.makeText(SearchVoucher.this, tr.getId(),
				...
				for (int j = 0; j < ColumnNameList.length; j++) {
					...
					ObjectAnimator colorFade = ObjectAnimator.ofObject(t, "backgroundColor",
					        new ArgbEvaluator(), Color.parseColor("#FBB117"), drawable.getColor());
					...
					builder.setItems(items, new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int pos) {
							if (pos == 0) {
								...
								// Toast.makeText(SearchVoucher.this,"result"+value,
								// Toast.LENGTH_SHORT).show();
								        ...
								        .setPositiveButton("Yes",
								                new DialogInterface.OnClickListener() {
									                public void onClick(DialogInterface dialog,
										                ...
										                deleteVoucher = (Boolean) transaction
										                        .deleteVoucher(params, client_id);
										                if (deleteVoucher.equals(true)) {
											                deletedflag = true;
											                deleted_vouchercode = vouchercode;
										                }
										                Object[] allvouchersparams = new Object[] {
										                        2, "", financialFromDate,
										                        financialToDate, "" };
		...
		params = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, 35);
	...
	protected void onResume() {
		...
		if (searchVoucherBy == 1) { // by reference number
			Object[] params = new Object[] { 1, searchByRefNumber, financialFromDate,
			        financialToDate, "" };
			getallvouchers(params);
			...
			Object[] params = new Object[] { 3, "", financialFromDate, financialToDate,
			        searchByNarration };
			getallvouchers(params);
	...
	protected Dialog onCreateDialog(int id) {
		...
		case FROM_DATE_DIALOG_ID:
			...
			return new DatePickerDialog(this, fromdatePickerListener,
			        Integer.parseInt(setfromyear), Integer.parseInt(setfrommonth) - 1,
			        Integer.parseInt(setfromday));

		case TO_DATE_DIALOG_ID:

			// set date picker as current date
			return new DatePickerDialog(this, todatePickerListener, Integer.parseInt(settoyear),
			        Integer.parseInt(settomonth) - 1, Integer.parseInt(settoday));
		}
		return null;
	}
	...
	private DatePickerDialog.OnDateSetListener fromdatePickerListener = new DatePickerDialog.OnDateSetListener() {
		...
		public void onDateSet(DatePicker view, int selectedYear, int selectedMonth, int selectedDay) {
			...
			givenfromDateString = String.format("%02d", day) + "-"
			        + String.format("%02d", month + 1) + "-" + (year - 1);
		}
	};
	...
	private DatePickerDialog.OnDateSetListener todatePickerListener = new DatePickerDialog.OnDateSetListener() {
		...
		public void onDateSet(DatePicker view, int selectedYear, int selectedMonth, int selectedDay) {
			...
			to_btn_ID.setText(new StringBuilder()
			        ...
			        .append(String.format("%02d", day)).append("-")
			        .append(String.format("%02d", month + 1)).append("-").append(year));
			givenToDateString = String.format("%02d", day) + "-" + String.format("%02d", month + 1)
			        + "-" + (year - 1);
		}
	};
