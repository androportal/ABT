...
The first step to get started in ``ABT`` is to create an organisation
  ...
  #. Fill the organisation details, if required.
  #. Press the ``Save`` button to save the organisation details or press ``Skip``. 

	.. image:: images/org_details.png
	   :name: ABT main page
	   :align: center

  #. Specify ``Preferences`` for projectwise accounting and account code.
  #. User can add new ``Projects`` and check the account code using the ``check-box``,if a manual account code has to be provided.
